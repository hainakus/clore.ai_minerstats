...
if [ ! $1 ]; then
  ...
  #################################£
  # Detect GPU's
  AMDDEVICE=$(timeout 40 sudo lspci -k | grep VGA | grep -vE "Kaveri|Beavercreek|Sumo|Wrestler|Kabini|Mullins|Temash|Trinity|Richland|Stoney|Carrizo|Raven|Renoir|Picasso|Van|RS880|Wani" | grep -c "AMD")
  if [ -z "$AMDDEVICE" ]; then
