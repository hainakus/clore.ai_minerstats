if ! screen -list | grep -q "dummy"; then
  ...
  then
    ...
    sudo echo "nameserver 1.1.1.1" > /run/resolvconf/interface/systemd-resolved 2>/dev/null
    sudo echo "nameserver 1.0.0.1" >> /run/resolvconf/interface/systemd-resolved 2>/dev/null
    sudo echo "nameserver 8.8.8.8" >> /run/resolvconf/interface/systemd-resolved 2>/dev/null
    sudo echo "nameserver 8.8.4.4" >> /run/resolvconf/interface/systemd-resolved 2>/dev/null
    #if [ ! -z "$GET_GATEWAY" ]; then
    #  sudo su -c "echo 'nameserver $GET_GATEWAY' > /run/systemd/resolve/stub-resolv.conf" 2>/dev/null
    #fi
    sudo echo "nameserver 1.1.1.1" > /run/systemd/resolve/stub-resolv.conf 2>/dev/null
    sudo echo "nameserver 1.0.0.1" >> /run/systemd/resolve/stub-resolv.conf 2>/dev/null
    sudo echo "nameserver 8.8.8.8" >> /run/systemd/resolve/stub-resolv.conf 2>/dev/null
    sudo echo "nameserver 8.8.4.4" >> /run/systemd/resolve/stub-resolv.conf 2>/dev/null
    sudo echo "options edns0" >> /run/systemd/resolve/stub-resolv.conf 2>/dev/null
  ...
  #fi
  sudo echo "nameserver 1.1.1.1" > /etc/resolv.conf 2>/dev/null
  sudo echo "nameserver 1.0.0.1" >> /etc/resolv.conf 2>/dev/null
  sudo echo "nameserver 8.8.8.8" >> /etc/resolv.conf 2>/dev/null
  sudo echo "nameserver 8.8.4.4" >> /etc/resolv.conf 2>/dev/null
  # China
  sudo echo "nameserver 114.114.114.114" >> /etc/resolv.conf 2>/dev/null
  sudo echo "nameserver 114.114.115.115" >> /etc/resolv.conf 2>/dev/null
  # IPV6
