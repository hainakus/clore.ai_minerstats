...
# Check SSH Keys
sudo screen -A -m -d -S sshgens sudo bash /home/minerstat/minerstat-os/core/ressh
# .bashrc
...
if [ -z "$1" ]; then
  AMDDEVICE=$(timeout 40 sudo lspci -k | grep VGA | grep -vE "Kaveri|Beavercreek|Sumo|Wrestler|Kabini|Mullins|Temash|Trinity|Richland|Stoney|Carrizo|Raven|Renoir|Picasso|Van|RS880" | grep -c "AMD")
  if [ -z "$AMDDEVICE" ]; then
