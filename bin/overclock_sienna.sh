...
if [ $1 ]; then
  ...
  if [ "$version" = "1.5.4" ]; then
    ...
    sudo /home/minerstat/.local/bin/upp -p /sys/class/drm/card$GPUID/device/pp_table set \
      ...
      smc_pptable/FanStopTemp=35 smc_pptable/FanStartTemp=20 smc_pptable/FanZeroRpmEnable=0 smc_pptable/FanTargetTemperature=$TT smc_pptable/FanTargetGfxclk=1000 smc_pptable/dBtcGbGfxDfllModelSelect=2 \
