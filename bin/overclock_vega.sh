...
if [ $1 ]; then
  ...
  then
    ...
    sudo su -c "echo 'm 1 $MEMCLOCK 950' > /sys/class/drm/card$GPUID/device/pp_od_clk_voltage" # @ 1100 mV default
    sudo su -c "echo 'm 2 $MEMCLOCK 955' > /sys/class/drm/card$GPUID/device/pp_od_clk_voltage" # @ 1100 mV default
    sudo su -c "echo 'm 3 $MEMCLOCK 965' > /sys/class/drm/card$GPUID/device/pp_od_clk_voltage" # @ 1100 mV default
