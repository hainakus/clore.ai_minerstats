...
if [ ! $1 ]; then
  ...
  if [ "$RESULT" = "0" ]; then
    #################################£
    # Detect GPU's
    AMDDEVICE=$(timeout 40 sudo lspci -k | grep -E "VGA|Display" | grep -vE "Kaveri|Beavercreek|Sumo|Wrestler|Kabini|Mullins|Temash|Trinity|Richland|Stoney|Carrizo|Raven|Renoir|Picasso|Van|RS880|Wrestler|X1200|RS780L" | grep -c "AMD")
    if [ -z "$AMDDEVICE" ]; then
