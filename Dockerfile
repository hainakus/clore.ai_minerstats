...
CMD  timeout 20 sudo nvidia-settings -a GPUPowerMizerMode=1 -c :0 2>/dev/null

CMD tmux new-session -s hainakus -d 'cd /home/minerstat/minerstat-os/; node --max-old-space-size=128 start' \; \
        resize-pane -U 5 \; \
        send-keys C-a M-3 \;
